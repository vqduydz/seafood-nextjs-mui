...
const CategoriesSlider = () => {
  ...
  const handleClick = (slug: string, event: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    const newHref = `/menu?category=${slug}`;
          ...
          {catalogs.map((item, index: number) => {
            ...
            return (
              <SwiperSlide
                ...
                style={{ padding: '8px 24px', fontSize: '1.5rem', color: '#e0592a', fontWeight: 900 }}
              >
                <Link href={`/menu?category=${slug}`} onClick={(e) => handleClick(`${slug}`, e)}>
                  {/* <Button
                    scale
                    // text_e
                    text
